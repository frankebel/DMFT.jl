...
@testset "update hybridization function" begin
    @testset "pole" begin
        Δ0 = Pole([1.0, 5.0], [0.1, 0.2])
        μ = Σ_H = 5.0 # cancel in PHS, take any value
        Σ = Pole([2.0, 4.0, 7.0], [0.5, 3.0, 0.05])
        ...
        @test typeof(Δ) === Pole{Vector{Float64},Vector{Float64}}
        @test Δ.a == [1.0, 5.0]
        ...
        # # PHS in → PHS out
        Δ0 = Pole([-0.5, 0.0, 0.5], [0.25, 2.0, 0.25])
        Σ = Pole([-2.125, 2.125], [0.7, 0.7])
        Δ = update_hybridization_function(Δ0, μ, Σ_H, Σ)
